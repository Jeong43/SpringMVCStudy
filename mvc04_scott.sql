...
, BASICPAY		NUMBER					-- 기본급
, EXTRAPAY		NUMBER					-- 수당
, CONSTRAINT EPLOYEE_ID_PK PRIMARY KEY(EMPLOYEEID)
, CONSTRAINT EPLOYEE_DEPARTMENTID_FK FOREIGN KEY(DEPARTMENTID)
			 REFERENCES DEPARTMENT(DEPARTMENTID)
, CONSTRAINT EPLOYEE_POSITIONID_FK FOREIGN KEY(POSITIONID)
			 REFERENCES POSITION(POSITIONID)
, CONSTRAINT EPLOYEE_REGIONID_FK FOREIGN KEY(REGIONID)
			 REFERENCES REGION(REGIONID)
, CONSTRAINT EPLOYEE_LUNAR_CK CHECK(LUNAR=0 OR LUNAR=1)
);
...
VALUES(EMPLOYEESEQ.NEXTVAL, '김진희', CRYPTPACK.ENCRYPT('9903202234567','9903202234567')
      , TO_DATE('1999-03-20', 'YYYY-MM-DD'), 0, '010-7389-9032', 1, 1, 1, 1500000, 1500000);  --백오십만, 백오십만
--==>> 1 행 이(가) 삽입되었습니다.
...
SELECT E.EMPLOYEEID, E.NAME, E.SSN, TO_CHAR(E.BIRTHDAY, 'YYYY-MM-DD') AS BIRTHDAY
     ...
     , (SELECT P.POSITIONNAME
        FROM POSITION P
        WHERE P.POSITIONID = E.POSITIONID) AS POSITIONNAME
     , E.REGIONID
     , (SELECT R.REGIONNAME
        FROM REGION R
        WHERE R.REGIONID = E.REGIONID) AS REGIONNAME
     , E.BASICPAY
     , EXTRAPAY
...
--○ 지역 데이터 조회 쿼리문 작성(지역 데이터 삭제 가능 여부 확인)
SELECT R.REGIONID, R.REGIONNAME
     , (SELECT COUNT(*)
        FROM EMPLOYEE E
        WHERE E.REGIONID = R.REGIONID) AS DELCHECK
FROM REGION R;
--==>>
/*
1	서울	1
...
SELECT P.POSITIONID, P.POSITIONNAME
     , (SELECT COUNT(*)
        FROM EMPLOYEE E
        WHERE E.POSITIONID = P.POSITIONID) AS DELCHECK
FROM POSITION P;
...
SELECT D.DEPARTMENTID, D.DEPARTMENTNAME
     , (SELECT COUNT(*)
        FROM EMPLOYEE E
        WHERE E.DEPARTMENTID = D.DEPARTMENTID) AS DELCHECK
FROM DEPARTMENT D;
...
-- 이와 관련하여 EMPLOYEEID(사원아이디)와 SSN(주민번호) 뒷자리의 숫자를 이용할 수 있도록 구성한다.
...
SELECT SUBSTR(TO_CHAR(CRYPTPACK.DECRYPT(SSN, '9903202234567')), 1, 6) AS SSN1
     , SUBSTR(TO_CHAR(CRYPTPACK.DECRYPT(SSN, '9903202234567')), 7, 6) AS SSN2
FROM EMPLOYEE;
...
--○ 컬럼 추가 → GRADE → 기본값을 1(일반 사원)으로 구성,  0은 관리자로 구성
ALTER TABLE EMPLOYEE
...
--○ 일반 사원 로그인 쿼리문(ID→EMPLOYEEID, PW→SSN2)
SELECT NAME
FROM EMPLOYEE
...
WHERE EMPLOYEEID='4'
  AND ( SELECT SSN2
        FROM EMPLOYEE
        WHERE EMPLOYEEID='4') = CRYPTPACK.ENCRYPT('2234561', '2234561');
--==>> 조회결과 없음 → 로그인 실패
...
--○ 관리자 로그인 쿼리문(ID→EMPLOYEEID, PW→SSN2)
SELECT NAME
FROM EMPLOYEE
WHERE EMPLOYEEID='ID문자열'
  AND SSN2=( SELECT SSN2
...
INSERT INTO DEPARTMENT(DEPARTMENTID, DEPARTMENTNAME) 
VALUES(DEPARTMENTSEQ.NEXTVAL, '총무부')
;
--==>> 1 행 이(가) 삽입되었습니다.
...
INSERT INTO POSITION(POSITIONID, POSITIONNAME, MINBASICPAY)
VALUES(POSITIONSEQ.NEXTVAL, '전무', 5000000)
;
--==>> 1 행 이(가) 삽입되었습니다.
...
--------------------------------------------------------------------------------
SELECT EMPLOYEEID, NAME, SSN, TO_CHAR(BIRTHDAY, 'YYYY-MM-DD') AS BIRTHDAY
